import { updateDarrenOneMoveStatus } from './darren-generated-strategy-core.js';
...
function safeErrorCode(error) {
  return String(error?.code || 'darren_one_move_status_update_failed')
    .replace(/[^a-z0-9_:-]/gi, '')
    .slice(0, 120) || 'darren_one_move_status_update_failed';
}
...
export default async function handler(req, res) {
  ...
  if (!expectedCode || providedCode !== expectedCode) {
    return res.status(403).json({ ok: false, error: 'admin_one_move_status_access_denied' });
  }

  try {
    const updatedStrategy = await updateDarrenOneMoveStatus(req.body || {});

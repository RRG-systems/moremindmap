...
const SNAPSHOT_VERSION = 'DARREN-LEADERSHIP-INTELLIGENCE-V1';
const DEFAULT_ADMIN_CODE = 'MOREADMIN26';
const DARREN_PROFILE_ID = 'mm-20260527-6zshuaao';
...
function buildSnapshot({ generatedAt, profileLookup, assessmentContext, dashboardContext }) {
  ...
  return {
    ...
    limits: [
      ...
      'No Redis keys or environment values returned.',
      'No records modified.',
...
export default async function handler(req, res) {
  ...
  if (!process.env.REDIS_URL) {
    return res.status(500).json({ ok: false, error: 'admin_snapshot_not_configured' });
  }

  const parsedDarrenProfile = parseProfileId(DARREN_PROFILE_ID);
  if (!parsedDarrenProfile) {
    return res.status(500).json({ ok: false, error: 'darren_profile_id_invalid' });
  }
  ...
  try {
    ...
    if (!profileLookup.found) {
      return res.status(404).json({
        ok: false,
        error: 'darren_profile_not_found',
    ...
    console.error('[DARREN INTELLIGENCE SNAPSHOT] Error:', error.message);
    return res.status(500).json({
      ok: false,
      error: 'darren_intelligence_snapshot_failed'
    });
  } finally {
    redis.disconnect();
  }

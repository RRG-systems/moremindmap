import { createDarrenFutureMovementAssessment } from './darren-adaptive-loop-core.js';
...
function safeErrorCode(error) {
  return String(error?.code || 'darren_future_movement_assessment_failed')
    .replace(/[^a-z0-9_:-]/gi, '')
    .slice(0, 120) || 'darren_future_movement_assessment_failed';
}
...
export default async function handler(req, res) {
  ...
  if (!expectedCode || providedCode !== expectedCode) {
    return res.status(403).json({ ok: false, error: 'admin_future_movement_access_denied' });
  }

  try {
    const assessment = await createDarrenFutureMovementAssessment();

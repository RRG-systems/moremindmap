...
export function buildFiveFuturesOnlyPrompt({
  assessmentRecord,
  businessIntelligenceDraft,
  executiveDiagnosticBriefing,
  canonicalProfile,
  ...
  const ownerProfileId = assessmentRecord?.owner_profile_id;
  const system = [
    'You are generating only the MORE MindMap Business Assessment V1 Five Futures object.',
    'This is probability-weighted trajectory modeling, not planning copy.',
    'Return the five_futures_v1 JSON object only. Do not generate one_move_v1.',
    'The futures must be distinct, evidence-linked, probability-distributed, and grounded in the Executive Diagnostic and business model.',
    'Produce concise schema-complete JSON, not prose-heavy narrative.',
    'Do not invent financial numbers, team facts, production history, or market facts not present in the inputs.',
    'Return valid JSON only. No markdown fences.'
  ].join('\n');
  ...
  const userPayload = {
    task: 'Generate Five Futures only for a saved Business Assessment.',
    output_contract: {
      top_level_required_keys: ['five_futures_v1'],
      five_futures_v1: {
        version: FIVE_FUTURES_VERSION,
        generated_at: 'ISO timestamp',
        assessment_id: assessmentId,
        ...
        futures: REQUIRED_FUTURE_KEYS.map((key) => ({
          ...
          what_the_system_sees: 'diagnostic observation',
          trajectory_logic: 'why this future forms',
          evidence: ['3-5 specific evidence items from answers, draft, briefing, profile, model'],
          behavioral_drivers: ['behavioral factors'],
          business_drivers: ['business factors'],
          financial_drivers: ['financial factors or missing financial data'],
          risk_if_unchanged: 'risk for current/next/constraint futures when relevant',
...
export function buildOneMovePrompt({
  assessmentRecord,
  businessIntelligenceDraft,
  executiveDiagnosticBriefing,
  fiveFutures,
  ...
  const ownerProfileId = assessmentRecord?.owner_profile_id;
  const system = [
    'You are generating only the MORE MindMap Business Assessment V1 One Move object.',
    'Return the one_move_v1 JSON object only. Do not regenerate Five Futures.',
    'The One Move must be singular, specific, high leverage, behavior-aware, measurable, and grounded in the validated Five Futures.',
    'Produce concise schema-complete JSON, not prose-heavy narrative.',
    'Do not invent financial numbers, team facts, production history, or market facts not present in the inputs.',
    'Return valid JSON only. No markdown fences.'
  ].join('\n');
  ...
  const userPayload = {
    task: 'Generate One Move only for a saved Business Assessment.',
    output_contract: {
      top_level_required_keys: ['one_move_v1'],
      one_move_v1: {
        version: ONE_MOVE_VERSION,
        generated_at: 'ISO timestamp',
        assessment_id: assessmentId,
    ...
    one_move_rules: [
      'Use the validated five_futures_v1 object as the trajectory map.',
      'The One Move must shift probability mass from current/constraint trajectory toward optimized trajectory.',
      'The One Move must be singular, specific, high leverage, behavior-aware, and measurable in 30-90 days.',
      'Do not produce ten recommendations or a generic coaching checklist.',
      'Keep it practical and complete, but not padded.'
    ],
    validated_five_futures_v1: fiveFutures,
    executive_diagnostic_signal: {
      primary_constraint_snapshot: executiveDiagnosticBriefing?.primary_constraint_snapshot,
      current_trajectory_signal: executiveDiagnosticBriefing?.current_trajectory_signal,
      confidence_snapshot: executiveDiagnosticBriefing?.confidence_snapshot,
      missing_data: executiveDiagnosticBriefing?.missing_data,
      sections: briefingSnapshot(executiveDiagnosticBriefing).sections
        .filter((section) =>
          [
            'executive_readout',
            'primary_constraint',
    ...
    business_intelligence_draft: businessIntelligenceDraft,
    canonical_profile_snapshot: compactCanonicalProfile(canonicalProfile),
    real_estate_one_move_model: {
      core_doctrine: realEstateBusinessModel?.core_doctrine,
      one_move_selection_model: realEstateBusinessModel?.one_move_selection_model,
      constraint_detection_framework: realEstateBusinessModel?.constraint_detection_framework,
      behavior_fusion_rules: realEstateBusinessModel?.behavior_fusion_rules
    }
  };

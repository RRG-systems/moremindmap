...
export function createNewBosModernizationService({
  ...
  return Object.freeze({
    ...
    async repairInvalidStage3VectorFree({
      profileId,
      suppliedToken = '',
      platformProtected = false,
      expectedCampaignSha256,
      ...
      const normalized = authorizeNewBosOperatorInspection({ config, profileId, suppliedToken, platformProtected });
      if (!platformProtected) throw new Error('new_bos_invalid_stage3_repair_requires_protected_candidate');
      if (typeof generator?.repairInvalidStage3VectorFree !== 'function') {
        throw new Error('new_bos_invalid_stage3_repair_operation_unavailable');
      }
      const desired = await desiredState(normalized);
      const current = await realizationStore.inspect({ profileId: normalized, desiredIdentity: desired.identity });
      if (current.state === 'current' || current.pointer) {
        throw new Error('new_bos_invalid_stage3_repair_realization_already_current');
      }
      return generator.repairInvalidStage3VectorFree({
        rawEvidence: desired.rawEvidence,
        providerModel: config.providerModel,
        realizationIdentity: desired.identity,
        expectedCampaignSha256,
    ...
    async inspectCompletedStage3Validation({
      profileId,
      suppliedToken = '',
      platformProtected = false,
      expectedCampaignSha256,
      ...
      const normalized = authorizeNewBosOperatorInspection({ config, profileId, suppliedToken, platformProtected });
      if (!platformProtected) throw new Error('new_bos_completed_stage3_diagnostic_requires_protected_candidate');
      if (typeof generator?.inspectCompletedStage3Validation !== 'function') {
        throw new Error('new_bos_completed_stage3_diagnostic_unavailable');
      }
      const desired = await desiredState(normalized);
      const current = await realizationStore.inspect({ profileId: normalized, desiredIdentity: desired.identity });
      if (current.state === 'current' || current.pointer) {
        throw new Error('new_bos_completed_stage3_diagnostic_realization_already_current');
      }
      return generator.inspectCompletedStage3Validation({
        rawEvidence: desired.rawEvidence,
        providerModel: config.providerModel,
        realizationIdentity: desired.identity,
        expectedCampaignSha256,
    ...
    async classifyCompletedStage3SemanticRejection({
      profileId,
      suppliedToken = '',
      platformProtected = false,
      expectedCampaignSha256,
      ...
      const normalized = authorizeNewBosOperatorInspection({ config, profileId, suppliedToken, platformProtected });
      if (!platformProtected) throw new Error('new_bos_completed_stage3_classification_requires_protected_candidate');
      if (typeof generator?.inspectCompletedStage3Validation !== 'function') {
        throw new Error('new_bos_completed_stage3_classification_unavailable');
      }
      const desired = await desiredState(normalized);
      const current = await realizationStore.inspect({ profileId: normalized, desiredIdentity: desired.identity });
      if (current.state === 'current' || current.pointer) {
        throw new Error('new_bos_completed_stage3_classification_realization_already_current');
      }
      return generator.inspectCompletedStage3Validation({
        rawEvidence: desired.rawEvidence,
        providerModel: config.providerModel,
        ...
        expectedRequestSha256,
        expectedProviderResponseIdSha256,
        persistSemanticRejection: true,
      });
    },
    async replaceSemanticRejectedStage3({
      profileId,
      suppliedToken = '',
      platformProtected = false,
      expectedCampaignSha256,
      ...
      const normalized = authorizeNewBosOperatorInspection({ config, profileId, suppliedToken, platformProtected });
      if (!platformProtected) throw new Error('new_bos_semantic_rejected_stage3_replacement_requires_protected_candidate');
      if (typeof generator?.replaceSemanticRejectedStage3 !== 'function') {
        throw new Error('new_bos_semantic_rejected_stage3_replacement_unavailable');
      }
      const desired = await desiredState(normalized);
      const current = await realizationStore.inspect({ profileId: normalized, desiredIdentity: desired.identity });
      if (current.state === 'current' || current.pointer) {
        throw new Error('new_bos_semantic_rejected_stage3_replacement_realization_already_current');
      }
      return generator.replaceSemanticRejectedStage3({
        rawEvidence: desired.rawEvidence,
        providerModel: config.providerModel,
        realizationIdentity: desired.identity,
        expectedCampaignSha256,
    ...
    async replaceStage3RequestContractV2({
      profileId,
      suppliedToken = '',
      platformProtected = false,
      expectedCampaignSha256,
      ...
      const normalized = authorizeNewBosOperatorInspection({ config, profileId, suppliedToken, platformProtected });
      if (!platformProtected) throw new Error('new_bos_stage3_request_contract_v2_requires_protected_candidate');
      if (typeof generator?.replaceStage3RequestContractV2 !== 'function') {
        throw new Error('new_bos_stage3_request_contract_v2_unavailable');
      }
      const desired = await desiredState(normalized);
      const current = await realizationStore.inspect({ profileId: normalized, desiredIdentity: desired.identity });
      if (current.state === 'current' || current.pointer) {
        throw new Error('new_bos_stage3_request_contract_v2_realization_already_current');
      }
      return generator.replaceStage3RequestContractV2({
        rawEvidence: desired.rawEvidence,
        providerModel: config.providerModel,
        realizationIdentity: desired.identity,
        expectedCampaignSha256,

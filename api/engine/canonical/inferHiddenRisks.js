...
export function inferHiddenRisks(vectorScores, stressPatterns, analyzedResponses, contradictions) {
  ...
  if (vectorScores.horizon > 0.65 && vectorScores.velocity < 0.45) {
  ...
  if (vectorScores.signal < 0.40 && stressPatterns?.pattern_type === 'narrowing') {
  ...
  if (vectorScores.vector > 0.65 && vectorScores.horizon < 0.45) {
  ...
  if (vectorScores.vector > 0.65 && vectorScores.velocity > 0.65 && business_reality?.word_count > 150) {
  ...
  if (stall_patterns?.frustrations?.includes('relational') && vectorScores.signal < 0.35) {
  ...
  if (vectorScores.signal < 0.35 && vectorScores.vector > 0.65) {

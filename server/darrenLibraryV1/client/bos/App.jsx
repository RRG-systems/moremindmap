...
import { VECTORS, CHAPTERS, QUESTIONS } from '../engine/design.js';
import './style.css';
...
async function request(path, options) {
  ...
  if (!response.ok) throw new Error(body.message || 'We could not load this reading. Please try again.');
  return body;
}
...
function Feedback() {
  ...
  async function save(event) {
    ...
    setStatus('Preview only. Your selection does not change this saved report.'); setSaving(false);
  }
  return <div className="feedback"><span>Does this sound like you? · Demo interaction</span><div className="feedback-choices">{[['fits','That fits'],['partly','Partly'],['misses','That misses me']].map(([id,label]) => <button key={id} aria-pressed={choice === id} onClick={() => {setChoice(id); setStatus('');}}>{label}</button>)}</div>{choice && <form onSubmit={save}><label>What should MORE understand?<textarea value={text} onChange={e => setText(e.target.value)} maxLength={2000} placeholder="Add a detail, an exception, or your side of the story."/></label><button type="submit" className="solid" disabled={saving}>{saving ? 'Saving…' : 'Try response'}</button></form>}<p role="status">{status}</p></div>;
}
...
export default function App() {
 ...
 useEffect(()=>{request('/darren-library/api/report/bos/'+new URLSearchParams(location.search).get('athlete')).then(setData).catch(e=>setError(e.message));},[]);
 ...
 if(!artifact)return <main className="waiting"><h1>{error||'Opening your personality profile…'}</h1><a href="/darren-library/library#bos">Back to the library</a></main>;
 return <><header className="topbar"><a className="brand" href="/darren-library/library#bos">MORE<span> ATHLETE</span></a><a href="/darren-library/library#bos">← All athlete reports</a></header>
      <section className="hero"><div><span className="eyebrow">Your Behavioral Operating System</span><h1>{artifact.subject.name.split(' ')[0]},<span>this is you.</span></h1><p>A closer look at what makes you who you are.</p></div><div className="identity"><span>{artifact.subject.age} · {artifact.subject.sport}</span><code>{artifact.mm}</code><button className="text-link" onClick={() => setModal({ type:'answers' })}>Your answers ↗</button><button className="text-link" onClick={() => window.print()}>Print / save PDF ↗</button></div></section>
...
<footer className="site-footer"><a href="/darren-library/library#apa">Explore the APA reports →</a><a href="/darren-library/library#bos">← All athlete reports</a></footer>

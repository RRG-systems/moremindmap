...
export function createAthleteConsultingV2Handler({ env = globalThis.process?.env || {}, redis: injectedRedis, authenticate, coach: injectedCoach, transport } = {}) {
  return async (req, res) => {
    try {
      ...
      if (kind !== 'action' || method !== 'POST') return send(res, 404, { error: 'NOT_FOUND' });

...
export function coachingInput(bundle,state,task){return {task,athlete:bundle.person,full_youth_bos:bundle.bos.reading,bos_evidence:bundle.bos.evidence,bos_answers:bundle.bos_source.answers,bos_validation_feedback:{source:'Athlete feedback on the unchanged original reading; self-report context, not independent validation or a revised personality score.',entries:Object.values(bundle.bos_feedback||{})},full_youth_apa:{...bundle.apa,receipts:undefined,audit:undefined,receipt:undefined,bos_sources:undefined},saved_plan:state.plan,pending_draft:state.draft,approved_learning:state.learning,previous_sessions:state.sessions.slice(-6),conversation:state.messages.slice(-50),visible_view:state.view,speaker:state.speaker,now:new Date().toISOString(),session_status:state.status};}

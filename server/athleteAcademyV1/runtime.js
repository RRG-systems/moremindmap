...
export function createAcademyRedis(url,{ca}={}){const parsed=new URL(url);let tls;if(parsed.protocol==='rediss:'){requireValue(typeof ca==='string'&&ca.includes('-----BEGIN CERTIFICATE-----'),'TLS_CA_REQUIRED',503);tls={ca};}const redis=new Redis(url,{maxRetriesPerRequest:0,enableOfflineQueue:false,lazyConnect:false,autoResendUnfulfilledCommands:false,retryStrategy:attempt=>Math.min(100*attempt,2000),...(tls?{tls}:{})});redis.on('error',()=>{});return redis;}
let runtime;
export function getAcademyRuntime(env=process.env){if(runtime)return runtime;requireValue(env.ATHLETE_ACADEMY_ENABLED==='1','ATHLETE_ACADEMY_NOT_ACTIVE',503);const url=env.ATHLETE_ACADEMY_REDIS_URL||env.REDIS_URL;requireValue(url,'ACADEMY_STORAGE_REQUIRED',503);const parsed=new URL(url);requireValue(parsed.protocol==='rediss:'||(env.ATHLETE_ACADEMY_LOCAL_PREVIEW==='1'&&['127.0.0.1','localhost'].includes(parsed.hostname)),'TLS_STORAGE_REQUIRED',503);const redis=createAcademyRedis(url,{ca:env.ATHLETE_ACADEMY_REDIS_CA_PEM});runtime=createAcademyRuntime({env,redis});return runtime;}

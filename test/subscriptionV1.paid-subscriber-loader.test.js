...
function loaderFor({
  ...
  const loader = createPaidSubscriberLoader({
    ...
    // The legacy generic fixture intentionally predates governed vertical
    // selection. Keep it on an explicit test-only library; the production
    // resolver now rejects unverified/unsupported vertical inference.
    resolveSourceLibrary: () => pinnedSubscriptionSources(),
    createTransport: () => {
...
test('paid subscriber projection removes generic runtime assignment prose while preserving business-model language', () => {
  const baseEnvelope = completedEnvelope();
  const artifact = clone(baseEnvelope.artifact);
  artifact.customer_view_model.runtime_notes = [
    ...
    'backend: secret-arm-b',
    'engine: secret-arm-b',
    'deployment: eastus-prod',
    'generated by model alpha',
    'Open AI deployment eastus-prod',
    'model is alpha',
    'model was alpha',
    'provider is alpha',
    'provider was alpha',
    'backend is secret-arm-b',
    'engine is secret-arm-b',
    'deployment is eastus-prod',
    'using model alpha',
    'uses model alpha',
    'chosen model alpha',
    'assigned model alpha',
    'running on model alpha',
    'powered by model alpha',
    'served by alpha model',
    'alpha model handled this request',
    'runtime: secret-arm-b',
    'model=alpha',
    'provider=alpha',
    'backend=secret-arm-b',
    'engine=secret-arm-b',
    ...
    'assigned alpha model',
    'running on alpha model',
    'powered by alpha model',
    'used alpha model',
    'we used alpha model',
    'uses alpha model',
    'chose alpha model',
  ...
  artifact.customer_view_model.safe_business_notes = [
    'business model analysis',
    'operating model design',
    'model home sales',
    'service provider relationships',
    'deployment of staff capacity',
    'engine of business growth',
  ];
  artifact.customer_view_model.backend = 'alpha';
  artifact.customer_view_model.engine = 'secret-arm-b';
  artifact.customer_view_model.deployment = 'eastus-prod';
  artifact.customer_view_model.inference_backend = 'alpha';
  artifact.customer_view_model.inferenceBackend = 'alpha';
  artifact.customer_view_model.runtimeEngine = 'secret-arm-b';
  artifact.customer_view_model.modelDeployment = 'eastus-prod';
  artifact.customer_view_model['inference-backend'] = 'alpha';
  artifact.customer_view_model['runtime-engine'] = 'secret-arm-b';
  artifact.customer_view_model['model-deployment'] = 'eastus-prod';
  artifact.customer_view_model.backend_name = 'alpha';
  artifact.customer_view_model.engine_name = 'secret-arm-b';
  artifact.customer_view_model.deployment_id = 'eastus-prod';
  artifact.customer_view_model.inference_model = 'alpha';
  artifact.customer_view_model.runtime_model = 'alpha';
  artifact.customer_view_model.selected_model = 'alpha';
  artifact.customer_view_model.modelName = 'alpha';
  artifact.customer_view_model.providerName = 'alpha';
  artifact.customer_view_model.runtimeConfig = 'secret-arm-b';
  artifact.customer_view_model.inferenceProvider = 'alpha';
  artifact.customer_view_model.business_model = 'relationship-led';
  artifact.customer_view_model.business_model_analysis = 'relationship-led';
  artifact.customer_view_model.business_engine = 'relationship-led';
  artifact.customer_view_model.business_growth_engine = 'referrals';
  artifact.customer_view_model.causal_model = { mechanism: 'relationship trust' };
  artifact.customer_view_model.confidence_engine = 'measured';
  artifact.customer_view_model.deployment_of_staff_capacity = 'sequenced';
  artifact.customer_view_model.engine_of_business_growth = 'referrals';
  artifact.customer_view_model.growth_engine = 'referrals';
  artifact.customer_view_model.model_home = 'desert ridge';
  artifact.customer_view_model.model_home_sales = 'growing';
  artifact.customer_view_model.operating_model = 'owner-led';
  artifact.customer_view_model.operating_model_design = 'weekly review';
  artifact.customer_view_model.operating_engine = 'weekly review';
  artifact.customer_view_model.primary_engine = 'service';
  artifact.customer_view_model.service_provider = 'title partner';
  artifact.customer_view_model.service_provider_relationships = 'healthy';
  artifact.customer_view_model.staff_deployment = 'sequenced';
  artifact.customer_view_model.staff_deployment_capacity = 'three roles';
  artifact.lineage.customer_projection_sha256 = sha256Stable(artifact.customer_view_model);
  const envelope = buildLaunchSafeNewBaEnvelope({
    profileId: REALIZATION_PROFILE_ID,
    realizationIdentity: baseEnvelope.realization_identity,
    artifact,
    compatibility: baseEnvelope.compatibility,
    providerAccounting: baseEnvelope.provider_accounting,
    createdAt: baseEnvelope.created_at,
  });
  const projection = projectCompletedRealProfileToSubscription({
    scope: SCOPE,
    assessment_id: ASSESSMENT_ID,
    profile_lookup: canonicalProfile(PROFILE_ID),
    realization_record: envelope,
    bos_realization_record: DEFAULT_BOS_ENVELOPE,
    assertBusinessScope: () => true,
  });
  assert.equal(projection.base_view_model.runtime_notes, undefined);
  assert.deepEqual(projection.base_view_model.safe_business_notes,
    artifact.customer_view_model.safe_business_notes);
  assert.equal(projection.base_view_model.backend, undefined);
  assert.equal(projection.base_view_model.engine, undefined);
  assert.equal(projection.base_view_model.deployment, undefined);
  assert.equal(projection.base_view_model.inference_backend, undefined);
  assert.equal(projection.base_view_model.inferenceBackend, undefined);
  assert.equal(projection.base_view_model.runtimeEngine, undefined);
  assert.equal(projection.base_view_model.modelDeployment, undefined);
  assert.equal(projection.base_view_model['inference-backend'], undefined);
  assert.equal(projection.base_view_model['runtime-engine'], undefined);
  assert.equal(projection.base_view_model['model-deployment'], undefined);
  assert.equal(projection.base_view_model.backend_name, undefined);
  assert.equal(projection.base_view_model.engine_name, undefined);
  assert.equal(projection.base_view_model.deployment_id, undefined);
  assert.equal(projection.base_view_model.inference_model, undefined);
  assert.equal(projection.base_view_model.runtime_model, undefined);
  assert.equal(projection.base_view_model.selected_model, undefined);
  assert.equal(projection.base_view_model.modelName, undefined);
  assert.equal(projection.base_view_model.providerName, undefined);
  assert.equal(projection.base_view_model.runtimeConfig, undefined);
  assert.equal(projection.base_view_model.inferenceProvider, undefined);
  assert.equal(projection.base_view_model.business_model, 'relationship-led');
  assert.equal(projection.base_view_model.business_model_analysis, 'relationship-led');
  assert.equal(projection.base_view_model.business_engine, 'relationship-led');
  assert.equal(projection.base_view_model.business_growth_engine, 'referrals');
  assert.deepEqual(projection.base_view_model.causal_model, { mechanism: 'relationship trust' });
  assert.equal(projection.base_view_model.confidence_engine, 'measured');
  assert.equal(projection.base_view_model.deployment_of_staff_capacity, 'sequenced');
  assert.equal(projection.base_view_model.engine_of_business_growth, 'referrals');
  assert.equal(projection.base_view_model.growth_engine, 'referrals');
  assert.equal(projection.base_view_model.model_home, 'desert ridge');
  assert.equal(projection.base_view_model.model_home_sales, 'growing');
  assert.equal(projection.base_view_model.hero.modelDate, 'Jan 2, 2099');
  assert.equal(projection.base_view_model.operating_model, 'owner-led');
  assert.equal(projection.base_view_model.operating_model_design, 'weekly review');
  assert.equal(projection.base_view_model.operating_engine, 'weekly review');
  assert.equal(projection.base_view_model.primary_engine, 'service');
  assert.equal(projection.base_view_model.service_provider, 'title partner');
  assert.equal(projection.base_view_model.service_provider_relationships, 'healthy');
  assert.equal(projection.base_view_model.staff_deployment, 'sequenced');
  assert.equal(projection.base_view_model.staff_deployment_capacity, 'three roles');
});
...
test('paid subscriber loader fails closed on ownership, assessment, and completion drift', async (t) => {
  ...
  await t.test('a Real Estate cassette cannot acquire Loan Originator open-plan semantics before storage', () => {
    const realEstateEnvelope = completedCurrentAveryEnvelope();
    const artifact = clone(realEstateEnvelope.artifact);
    artifact.plan_135.plan_state = 'LO_OPEN_DRAFT';
    artifact.lineage.plan_sha256 = sha256Stable(artifact.plan_135);
    assert.throws(() => buildLaunchSafeNewBaEnvelope({
      profileId: REALIZATION_PROFILE_ID,
      realizationIdentity: realEstateEnvelope.realization_identity,
      artifact,
      compatibility: realEstateEnvelope.compatibility,
      providerAccounting: realEstateEnvelope.provider_accounting,
      createdAt: realEstateEnvelope.created_at,
  ...
  await t.test('realization identity and recorded BOS authority must remain cross-bound', async () => {
    ...
    recordedBosDrift.completeness.artifact_sha256 = recordedBosDrift.artifact_sha256;
...
test('paid subscriber loader rejects recorded BOS snapshot drift before relationship state or provider access', async (t) => {
  ...
  for (const [name, mutate] of [
    ...
    await t.test(`${name} drift is rejected`, async () => {
      ...
      drifted.completeness.artifact_sha256 = drifted.artifact_sha256;
      const counters = {};

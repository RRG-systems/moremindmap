...
test('public redaction removes direct runtime assignments without hiding safe business language', () => {
  const replacement = 'Private runtime configuration is not exposed.';
  for (const value of [
    'model used: alpha',
    'we used model alpha',
    ...
    assert.equal(redactPaidRuntimePayload({ ok: true, message: value }).message, replacement, value);
  }
  for (const value of [
    ...
    assert.equal(redactPaidRuntimePayload({ ok: true, message: value }).message, value, value);
  }
  assert.deepEqual(redactPaidRuntimePayload({
    ok: true,
    backend: 'alpha',
    engine: 'secret-arm-b',
    deployment: 'eastus-prod',
    inference_backend: 'alpha',
  }), { ok: true });
  assert.deepEqual(redactPaidRuntimePayload({
    ok: true,
    inferenceBackend: 'alpha',
    runtimeEngine: 'secret-arm-b',
    modelDeployment: 'eastus-prod',
    'inference-backend': 'alpha',
    'runtime-engine': 'secret-arm-b',
    'model-deployment': 'eastus-prod',
    backend_name: 'alpha',
    engine_name: 'secret-arm-b',
    deployment_id: 'eastus-prod',
    inference_model: 'alpha',
    runtime_model: 'alpha',
    selected_model: 'alpha',
    modelName: 'alpha',
    providerName: 'alpha',
    runtimeConfig: 'secret-arm-b',
    inferenceProvider: 'alpha',
  }), { ok: true });
  const safeBusinessEngineFields = {
    business_model: 'relationship-led',
    business_model_analysis: 'relationship-led',
    business_engine: 'relationship-led',
    business_growth_engine: 'referrals',
    causal_model: { mechanism: 'relationship trust' },
    confidence_engine: 'measured',
    deployment_of_staff_capacity: 'sequenced',
    engine_of_business_growth: 'referrals',
    growth_engine: 'referrals',
    model_home: 'desert ridge',
    model_home_sales: 'growing',
    modelDate: 'Jan 2, 2099',
    operating_model: 'owner-led',
    operating_model_design: 'weekly review',
    operating_engine: 'weekly review',
    primary_engine: 'service',
    service_provider: 'title partner',
    service_provider_relationships: 'healthy',
    staff_deployment: 'sequenced',
    staff_deployment_capacity: 'three roles',
    view_model: { headline: 'Customer-visible business view.' },
  };
  assert.deepEqual(redactPaidRuntimePayload(safeBusinessEngineFields), safeBusinessEngineFields);
});

test('public redaction preserves exact opaque CSRF controls without exempting ordinary prose', () => {
  const replacement = 'Private runtime configuration is not exposed.';
  const tokens = [
    `csrf-${'a'.repeat(16)}-o3-${'b'.repeat(24)}`,
    `csrf-${'a'.repeat(16)}-gpt-${'b'.repeat(24)}`,
    `csrf-${'a'.repeat(16)}-claude-3-${'b'.repeat(24)}`,
  ];

  for (const token of tokens) {
    assert.equal(redactPaidRuntimePayload({ ok: true, csrf_token: token }).csrf_token, token);
    assert.equal(redactPaidRuntimePayload({ ok: true, message: token }).message, replacement);
    assert.equal(redactPaidRuntimePayload({ ok: true, nested: { csrf_token: token } }).nested.csrf_token, replacement);
  }

  assert.equal(
    redactPaidRuntimePayload({ ok: true, customer_message: 'The request was handled by model alpha.' }).customer_message,
    replacement,
  );
});

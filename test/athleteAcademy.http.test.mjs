...
import {createAcademyRedis,createAcademyRuntime,getAcademyRuntime} from '../server/athleteAcademyV1/runtime.js';import {digest} from '../server/athleteAcademyV1/repository.js';
...
test('Academy runtime fails closed instead of falling back to the shared Production Redis binding',()=>{assert.throws(()=>getAcademyRuntime({...env,ATHLETE_ACADEMY_REDIS_URL:'',REDIS_URL:'redis://127.0.0.1:6394'}),/ACADEMY_STORAGE_REQUIRED/);});
test('cold storage connects before the first command without enabling offline replay',async()=>{const cold=createAcademyRedis('redis://127.0.0.1:6394',{lazyConnect:true});assert.equal(cold.status,'wait');const runtime=createAcademyRuntime({env:{...env,ATHLETE_ACADEMY_NAMESPACE:`more:athlete-academy:{test-${randomUUID()}}`},redis:cold,mailTransport:async()=>({status:'sent'})});await runtime.ready();assert.equal(cold.status,'ready');assert.equal(cold.options.enableOfflineQueue,false);assert.equal(cold.options.maxRetriesPerRequest,0);await cold.quit();});
...
test('only canonical, explicitly listed, and current Vercel deployment origins are accepted',()=>{const config=academyConfig({...env,ATHLETE_ACADEMY_ALLOWED_ORIGINS:'https://www.moremindmap.com',VERCEL_URL:'athlete-candidate.vercel.app'});assert.deepEqual([...config.allowedOrigins].sort(),['http://127.0.0.1:5321','https://athlete-candidate.vercel.app','https://www.moremindmap.com']);assert.equal(config.cohort.minimumAge,18);const youth=academyConfig({...env,ATHLETE_ACADEMY_REAL_YOUTH_ENABLED:'1'});assert.equal(youth.cohort.minimumAge,17);assert.equal(youth.cohort.guardianRequiredUnder,18);assert.throws(()=>academyConfig({...env,ATHLETE_ACADEMY_ALLOWED_ORIGINS:'https://*.moremindmap.com'}),/ACADEMY_ALLOWED_ORIGIN_INVALID/);});
test('existing server-only Resend bindings can be reused without copying their values',()=>{const aliasRuntime=createAcademyRuntime({env:{...env,RESEND_API_KEY:'',ATHLETE_ACADEMY_MAIL_FROM:'',MOREMINDMAP_SERVER_ONLY_PROFILE_OWNERSHIP_RESEND_API_KEY:'present-server-only',PUBLIC_PROFILE_OWNERSHIP_EMAIL_FROM:'MORE MindMap <synthetic@example.test>'},redis});assert.equal(aliasRuntime.config.mailEnabled,true);});

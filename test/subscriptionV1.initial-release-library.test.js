...
test('paid function bundles pinned libraries plus the exact synthetic QA runtime custody set',()=>{
 ...
 const deploymentIgnore=readFileSync(new URL('../.vercelignore',import.meta.url),'utf8');
 ...
 for(const required of [
  '!api/engine/subscriptionV1/sourceLibrary/**',
  '!docs/lo-cassette-2-final-canonical-authority-v1/**',
 ]){
  assert.ok(deploymentIgnore.includes(required),required);
  assert.ok(deploymentIgnore.lastIndexOf(required)>deploymentIgnore.lastIndexOf('*.md'),required);
  assert.ok(deploymentIgnore.lastIndexOf(required)>deploymentIgnore.lastIndexOf('*.txt'),required);
 }

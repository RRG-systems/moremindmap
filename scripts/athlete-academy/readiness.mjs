...
const checks={
  ...
  tls_redis:(e.ATHLETE_ACADEMY_REDIS_URL||'').startsWith('rediss://'),
  ...
  email_binding:e.ATHLETE_ACADEMY_MAIL_ENABLED==='1'&&Boolean((e.RESEND_API_KEY||e.MOREMINDMAP_SERVER_ONLY_PROFILE_OWNERSHIP_RESEND_API_KEY)&&(e.ATHLETE_ACADEMY_MAIL_FROM||e.PUBLIC_PROFILE_OWNERSHIP_EMAIL_FROM)),
  participation_version:Boolean(e.ATHLETE_ACADEMY_REVIEWED_POLICY_VERSION),
  adult_only_lock:e.ATHLETE_ACADEMY_REAL_YOUTH_ENABLED!=='1',
  worker_binding:e.ATHLETE_ACADEMY_WORKER_ENABLED==='1'&&(e.CRON_SECRET||'').length>=32,
};
const informational={youth_activation:e.ATHLETE_ACADEMY_REAL_YOUTH_ENABLED==='1'};
console.log(JSON.stringify({configurationReady:Object.values(checks).every(Boolean),checks,informational,proves:'Configuration presence only; hosted storage, mail, scheduler, browser and synthetic acceptance are separate.'},null,2));

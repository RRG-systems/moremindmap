...
for (const fixture of fixtures) {
  ...
  assert.equal(
    contract.contract_metadata.true_relationships_provenance_version,
    'true-relationships-provenance-v1'
  );
  assert.equal(
    contract.relationship_lake.true_relationships.provenance,
    'EXPLICIT'
  );
  assert.equal(
    contract.relationship_lake.true_relationships.temporal_state,
    'CURRENT'
  );

...
// Case 1: strong intelligence available + repaired semantics
{
  ...
  assert(streams.fallback_used === true, 'deterministic streams must set fallback_used=true', failures);
  assert(Boolean(streams.fallback_reason), 'streams fallback_reason required', failures);
  assert(
    streams.source_type === 'deterministic_normalized' || streams.source_type === 'legacy_fallback',
    `streams source_type must be deterministic/legacy, got ${streams.source_type}`,
    failures
  );
  assert(
    streams.intelligence_status === 'fallback' || streams.intelligence_status === 'partial',
    `streams must not claim available domain intelligence, got ${streams.intelligence_status}`,
    failures
  );
  assert(outflow.fallback_used === true, 'deterministic outflow must set fallback_used=true', failures);
  assert(Boolean(outflow.fallback_reason), 'outflow fallback_reason required', failures);
  assert(
    outflow.source_type === 'deterministic_normalized' || outflow.source_type === 'legacy_fallback',
    `outflow source_type must be deterministic/legacy, got ${outflow.source_type}`,
    failures
  );

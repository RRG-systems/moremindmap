...
function SideRail({ viewModel, active, navigate }) {
  return (
    <aside className="side-rail">
      ...
      <div className="rail-boundary"><span aria-hidden="true">▢</span><div><strong>Frozen assessment</strong><small>This map changes only when accepted business evidence changes.</small></div></div>
    </aside>
  )
}
...
function Overview({ viewModel, navigate, openObject, onHow }) {
  ...
  return (
    <main className="overview" data-layer="0">
      ...
      <footer className="snapshot-boundary"><span>▢</span><div><strong>This is a frozen business snapshot.</strong><p>It reflects the accepted evidence available at the model date.</p></div></footer>
    </main>
  )
}
...
export default function BusinessTwinApp({ viewModel }) {
  ...
  function navigate(destination) { setDrawerObjectId(null); setDrawerContext(null); setDisclosure(null); setActive(destination); globalThis.scrollTo({ top: 0, behavior: 'auto' }) }
